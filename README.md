...
- **config.json**: JSON 格式配置文件，支持相对路径
...
- 🔔 **系统托盘** - 最小化或关闭窗口时最小化到系统托盘，后台静默运行
- 🔄 **进程重启** - 一键重启 Aria2 进程
- 📥 **自动下载** - 首次运行时自动从 GitHub 下载 aria2c（带进度提示）

## 📁 文件结构

```
Aria2-Manager/
├── Aria2c_Manager.ps1      # 主管理脚本
├── 启动Aria2c管理工具.vbs   # VBS启动器（隐藏控制台窗口）
├── config.json             # JSON配置文件
├── aria2.conf              # Aria2 配置文件
├── aria2c.exe              # Aria2 核心（自动下载）
└── index.html              # AriaNg 界面
```
...
### 前置要求
- Windows 7 及以上版本
- PowerShell 5.1 及以上版本（Windows 7+ 自带）
...
2. **一键启动**
   - 双击运行 `启动Aria2c管理工具.vbs`（推荐，无控制台窗口）
   - 或直接运行 `Aria2c_Manager.ps1` 脚本
   - 首次运行会自动从 GitHub 下载 aria2c.exe
   - 程序启动后会自动在后台运行 Aria2c

3. **开始下载**
   - 在打开的管理界面中，使用 AriaNg 进行下载管理
   - 点击最小化或关闭按钮可将窗口最小化到系统托盘
   - Aria2 将继续在后台运行
...
| 版本 | 说明 |
|------|------|
| v0.2 | 重构版本：模块化设计、系统托盘、JSON配置、自动下载aria2c |
| v0.1 | 初始版本：基础管理功能 |
